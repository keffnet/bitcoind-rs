...
The daemon writes tracing output to `<datadir>/debug.log` by default and reports the resolved path through `getnetworkinfo` and `getrpcinfo`. Use `--debuglogfile=<path>` for another location, `--nodebuglogfile` to disable file output, `--printtoconsole=false` to silence the console, and `--shrinkdebugfile=false` to append across restarts.
