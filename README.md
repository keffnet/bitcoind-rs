...
It is not yet a drop-in replacement for every Bitcoin Core 31.1 behavior. In particular, the storage engine is append-only with JSON chain metadata and a lightweight JSON `peers.json` address table rather than Core's production database, mempool import/export uses this implementation's JSON format rather than Core's binary `mempool.dat`, pruning is intentionally disabled, UTXO snapshot files use this implementation's JSON format, and Electrum indexing is in-process rather than a separate electrs database. Miniscript-backed taproot-tree derivation and PSBT tree metadata are supported, but general miniscript satisfaction and taproot script-path signing are not complete. Full mainnet deployment still requires broader Core test-vector, reorg, fuzz, and interoperability testing.
