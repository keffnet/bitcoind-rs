...
The node holds an advisory exclusive lock on `<datadir>/.lock` for its lifetime, so a second instance cannot open the same chainstate and append-only stores concurrently.
