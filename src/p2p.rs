    ...
    loop {
        ...
        let message = if let Some(items) = take_getdata_batch(&mut pending_getdata) {
            ...
            _ = tx_request_interval.tick(), if version_received && verack_received && !reader.has_partial_frame() => {
                // Core evaluates fee-filter transitions in its regular
                // SendMessages loop. Refreshing here prevents a peer from
                // retaining the temporary IBD filter after the active tip
                // becomes current while an inventory announcement is queued.
                maybe_send_fee_filter(
                    node,
                    peer_id,
                    writer,
            ...
            Message::Inv(items) => {
                ...
                let late_orphan_transactions = items
                    ...
                    .filter_map(|item| {
                        ...
                        .then(|| {
                            node.orphan_transaction_for_inventory(item.hash, item.kind.uses_wtxid())
                        })
                        .flatten()
                    })
                    .collect::<Vec<_>>();
                {
                    ...
                    for item in &items {
                        let matching_kind = (wtxid_relay && item.kind.uses_wtxid())
                            || (!wtxid_relay
                                && item.kind.is_transaction()
                                && !item.kind.uses_wtxid());
                        if matching_kind {
                    ...
                    items
                        ...
                        .filter_map(|mut item| match item.kind {
                            ...
                            kind if kind.is_transaction() => {
                                ...
                                if node.orphan_has_inventory(item.hash, item.kind.uses_wtxid()) {
                                    return None;
                                }
...
fn transaction_fee_for_inventory(node: &Arc<Node>, item: &Inventory) -> Option<(u64, u64)> {
    let mempool = node.mempool.read();
    match item.kind {
        kind if kind.uses_wtxid() => mempool
...
mod tests {
    ...
    fn peer_counters_track_wire_traffic_and_pings() {
        ...
        node.record_bytes_received(7, 23, "future-message");
        ...
        assert_eq!(node.total_bytes_received(), 42);
        let peer = node.peer_infos().pop().expect("registered peer");
        assert_eq!(peer.bytes_sent, 42);
        assert_eq!(peer.bytes_received, 42);
        assert_eq!(peer.bytes_sent_per_msg.get("ping"), Some(&42));
        assert_eq!(peer.bytes_received_per_msg.get("tx"), Some(&19));
        assert_eq!(peer.bytes_received_per_msg.get("*other*"), Some(&23));
        assert!(!peer.bytes_received_per_msg.contains_key("future-message"));
