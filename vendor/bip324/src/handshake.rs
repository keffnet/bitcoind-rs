...
impl Handshake<Initialized> {
    ...
    #[cfg(feature = "std")]
    pub fn new(magic: impl Borrow<[u8; 4]>, role: Role) -> Result<Self, Error> {
...
impl<'a> Handshake<SentKey<'a>> {
    ...
    pub fn receive_key(
        ...
        let their_magic: [u8; 4] = their_key[..4]
            .try_into()
            .expect("64 byte array to have 4 byte prefix");

        if self.magic == their_magic {
            return Err(Error::V1Protocol);
        }

        self.receive_key_without_v1_check(their_key)
    }

    /// Process a remote key after the caller has performed transport
    /// detection itself.
    ///
    /// Bitcoin Core distinguishes v1 from v2 using the complete 16-byte v1
    /// preface. A v2 ElligatorSwift key can therefore legitimately begin
    /// with the four-byte network magic. The regular [`receive_key`] method
    /// retains its historical four-byte compatibility check for callers that
    /// do not perform that detection, while this method lets a transport
    /// implementation apply the exact preface rule before entering BIP324.
    pub fn receive_key_without_v1_check(
        self,
        their_key: [u8; NUM_ELLIGATOR_SWIFT_BYTES],
    ) -> Result<Handshake<ReceivedKey<'a>>, Error> {
...
mod tests {
    ...
    fn test_handshake_send_key() {
        ...
        let handshake = Handshake::<Initialized>::new(MAGIC, Role::Initiator).unwrap();
    ...
    fn test_v1_protocol_detection() {
        ...
        let handshake = Handshake::<Initialized>::new([0u8; 4], Role::Responder).unwrap();
        let handshake = handshake.send_key(None, &mut buffer).unwrap();

        let mut v1_testnet_key = [0u8; NUM_ELLIGATOR_SWIFT_BYTES];
        v1_testnet_key[..4].copy_from_slice(&[0u8; 4]);
